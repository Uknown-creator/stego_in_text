...
/// @brief Участник обмена по протоколу Диффи-Хеллмана.
///
/// Использует фиксированную группу MODP из RFC 3526 (2048 бит). Приватный
/// ключ хранится в объекте и может быть сохранён/загружен из файла, что даёт
/// клиенту устойчивую криптографическую личность ("файл с ключом").
class DiffieHellman {
   public:
...
/// @brief Симметричный шифр AES-256 в режиме GCM (аутентифицированное шифрование).
///
/// Вектор инициализации (IV) генерируется случайно при каждом вызове encrypt()
/// и помещается в начало результата, чтобы decrypt() мог его извлечь.
class AesCipher {
   public:
